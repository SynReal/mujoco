...
class USDFlex(USDObject):
    ...
    def _enable_deformable_body(self, stage, prim_path):
        ...
        prim.CreateAttribute(
            "physxDeformable:deformableEnabled", Sdf.ValueTypeNames.Bool
        ).Set(True)
        prim.CreateAttribute(
            "physxDeformable:solverPositionIterationCount", Sdf.ValueTypeNames.Int
        ).Set(16)
        prim.CreateAttribute(
            "physxDeformable:selfCollision", Sdf.ValueTypeNames.Bool
        ).Set(True)
